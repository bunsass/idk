...
-- Add after all Tabs and Sections are defined (around line 1217):
...
Tabs.AutoPlay = AutoPlaySection:Tab({ Title = "Auto Play", Icon = "gamepad" })
local GB_AutoPlay = adaptTab(Tabs.AutoPlay)

GB_AutoPlay:Space()
GB_AutoPlay:Divider()
GB_AutoPlay:Section({ Title = "🎮 Auto Play System (Independent)" })

GB_AutoPlay:Paragraph({
    Title = "Smart Unit Placement",
    Desc = "Automatically places units from your slots (1-6) at a set percentage of the path. Places units you can afford in order. Works independently from macros!"
})

addToggle(GB_AutoPlay, "AutoPlayToggle", "✨ Enable Auto Play", getgenv().Config.toggles.AutoPlayToggle or false, function(val)
    getgenv().AutoPlayEnabled = val
    getgenv().Config.toggles.AutoPlayToggle = val
    saveConfig(getgenv().Config)
    if val then
        notify("Auto Play", "Enabled - Will auto-place on map change", 5)
    else
        notify("Auto Play", "Disabled", 3)
    end
end)

GB_AutoPlay:AddInput("AutoPlayPercentage", {
    Text = "Path Percentage (0-100%)",
    Default = getgenv().Config.inputs.AutoPlayPercentage or "50",
    Numeric = true,
    Finished = true,
    Placeholder = "50 = middle of path",
    Callback = function(value)
        ...
        if num and num >= 0 and num <= 100 then
            ...
            saveConfig(getgenv().Config)
            notify("Auto Play", "Set to " .. num .. "% of path", 3)
        end
    end
})

addToggle(GB_AutoPlay, "AutoPlayUpgradeToggle", "🔼 Auto Upgrade After Place", getgenv().Config.toggles.AutoPlayUpgradeToggle or false, function(val)
    getgenv().AutoPlayUpgradeAfterPlace = val
    getgenv().Config.toggles.AutoPlayUpgradeToggle = val
    saveConfig(getgenv().Config)
    notify("Auto Upgrade", val and "Enabled" or "Disabled", 3)
end)

GB_AutoPlay:AddInput("AutoPlayUpgradeLevel", {
    Text = "Upgrade Level (0-50)",
    Default = getgenv().Config.inputs.AutoPlayUpgradeLevel or "0",
    Numeric = true,
    Finished = true,
    Placeholder = "Max level to upgrade to",
    Callback = function(value)
        local num = tonumber(value)
        if num and num >= 0 and num <= 50 then
            getgenv().AutoPlayUpgradeLevel = num
            getgenv().Config.inputs.AutoPlayUpgradeLevel = tostring(num)
            saveConfig(getgenv().Config)
            notify("Auto Upgrade", "Set to level " .. num, 3)
        end
    end
})

GB_AutoPlay:AddButton("🚀 Manual Trigger", function()
    if not getgenv().AutoPlayEnabled then
        notify("Auto Play", "Please enable Auto Play first", 3)
        return
    end
    notify("Auto Play", "Starting placement...", 3)
    getgenv().TriggerAutoPlay()
end)

GB_AutoPlay:AddButton("📍 Preview Location", function()
    pcall(function()
        local waypoints = scanMapWaypoints()
        if #waypoints < 2 then
            notify("Auto Play", "No waypoints found - join a game first", 3)
            return
        end
        local position, direction = getPositionAtPercentage(waypoints, getgenv().AutoPlayPercentage)
        if position then
            -- Create preview marker
            local marker = Instance.new("Part")
            marker.Size = Vector3.new(6, 12, 6)
            marker.Color = Color3.fromRGB(0, 255, 0)
            marker.Material = Enum.Material.Neon
            marker.Anchored = true
            marker.CanCollide = false
            marker.Transparency = 0.3
            marker.CFrame = CFrame.new(position, position + direction)
            marker.Parent = workspace
            -- Arrow
            local arrow = Instance.new("Part")
            arrow.Size = Vector3.new(3, 1, 10)
            arrow.Color = Color3.fromRGB(255, 255, 0)
            arrow.Material = Enum.Material.Neon
            arrow.Anchored = true
            arrow.CanCollide = false
            arrow.CFrame = marker.CFrame * CFrame.new(0, 7, 0)
            arrow.Parent = marker
            -- Text label
            ...
            textLabel.Text = getgenv().AutoPlayPercentage .. "% of Path"
            textLabel.TextColor3 = Color3.new(1, 1, 1)
            textLabel.TextScaled = true
            textLabel.Font = Enum.Font.SourceSansBold
            textLabel.Parent = billboardGui
            local pathLength = getPathLength(waypoints)
            notify("Preview", getgenv().AutoPlayPercentage .. "% | Path: " .. math.floor(pathLength) .. " studs", 5)
            task.wait(8)
            marker:Destroy()
        end
    end)
...
GB_AutoPlay:AddButton("🗺️ Scan Current Map", function()
    pcall(function()
        local mapName = getCurrentMap()
        local waypoints = scanMapWaypoints()
        local pathLength = getPathLength(waypoints)
        if mapName and #waypoints > 0 then
            notify("Map Scan", mapName .. ": " .. #waypoints .. " waypoints | " .. math.floor(pathLength) .. " studs", 5)
        else
            notify("Map Scan", "No map detected or no waypoints found", 3)
        end
    end)
end)
...
GB_AutoPlay:Paragraph({
    Title = "💡 How It Works",
    Desc = "1. Set your path percentage (0-100%)\n2. Enable Auto Play\n3. Join a game - it will auto-place units from Slot 1-6\n4. Only places units you can afford\n5. Automatically detects map changes!"
})
...
-- ============================================================================
-- STANDALONE AUTO PLAY SYSTEM
-- Automatically places units at percentage of path based on slots & money
-- ============================================================================

getgenv().AutoPlayEnabled = false
getgenv().AutoPlayPercentage = 50
getgenv().AutoPlayPathWaypoints = {}
getgenv().AutoPlayCurrentMap = nil
getgenv().AutoPlayPlacedUnits = {}
getgenv().AutoPlayUpgradeAfterPlace = false
getgenv().AutoPlayUpgradeLevel = 0
...
local function getCurrentMap()
    local ok, result = pcall(function()
        ...
        local mapName = map:FindFirstChild("MapName")
        return mapName and mapName.Value or nil
    end)
    return ok and result or nil
end

-- Get waypoints from map
local function scanMapWaypoints()
    local waypoints = {}
    pcall(function()
        local map = workspace:FindFirstChild("Map")
        if not map then return end
        
        -- Try different waypoint location names
        local waypointFolder = map:FindFirstChild("Waypoints") or 
                              map:FindFirstChild("Path") or 
                              map:FindFirstChild("Points") or
                              map:FindFirstChild("WalkPath")
        
        if waypointFolder then
            local children = waypointFolder:GetChildren()
            local namedWaypoints = {}
            
            for i = 1, #children do
                local wp = children[i]
                if wp:IsA("BasePart") then
                    local num = tonumber(wp.Name:match("%d+"))
                    if num then
                        namedWaypoints[num] = wp.Position
                    else
            ...
            -- If we found numbered waypoints, sort them
            if next(namedWaypoints) then
                waypoints = {}
                local keys = {}
                for k in pairs(namedWaypoints) do table.insert(keys, k) end
                table.sort(keys)
                for _, k in ipairs(keys) do
                    table.insert(waypoints, namedWaypoints[k])
                end
            end
        end
    end)
...
local function getPathLength(waypoints)
    ...
    for i = 1, #waypoints - 1 do
        total = total + (waypoints[i+1] - waypoints[i]).Magnitude
    end
    return total
end

-- Get position at percentage of path
local function getPositionAtPercentage(waypoints, percentage)
    if #waypoints < 2 then return nil, nil end
    
    local totalLength = getPathLength(waypoints)
    local targetDistance = totalLength * (percentage / 100)
    
    local currentDistance = 0
    for i = 1, #waypoints - 1 do
        local start = waypoints[i]
        local endPos = waypoints[i+1]
        local segmentLength = (endPos - start).Magnitude
        
        if currentDistance + segmentLength >= targetDistance then
            local remaining = targetDistance - currentDistance
            local alpha = remaining / segmentLength
            
            local position = start:Lerp(endPos, alpha)
    ...
    return waypoints[#waypoints], (waypoints[#waypoints] - waypoints[#waypoints - 1]).Unit
end

-- Get unit cost from tower info
local function getUnitCost(unitName)
    local cost = 0
    pcall(function()
        local towerInfoPath = game:GetService("ReplicatedStorage"):FindFirstChild("Modules"):FindFirstChild("TowerInfo")
        if not towerInfoPath then return end
        
        local towerModule = towerInfoPath:FindFirstChild(unitName)
        if not towerModule or not towerModule:IsA("ModuleScript") then return end
        
        local info = require(towerModule)
        if info and info[0] and info[0].Cost then
            cost = info[0].Cost
        end
    end)
    return cost
...
-- Get player's current slots
local function getPlayerSlots()
    local slots = {}
    pcall(function()
        local clientDataModule = game:GetService("ReplicatedStorage"):FindFirstChild("Modules"):FindFirstChild("ClientData")
        if not clientDataModule or not clientDataModule:IsA("ModuleScript") then return end
...
local function getCurrentCash()
    local cash = 0
    pcall(function()
        cash = game:GetService("Players").LocalPlayer.Cash.Value
    end)
    return cash
end

-- Find placement remote
local function getPlacementRemote()
    local remote = nil
    pcall(function()
        local RS = game:GetService("ReplicatedStorage")
        
        -- Try cached first
        if getgenv().MacroRemoteCache then
            for name, r in pairs(getgenv().MacroRemoteCache) do
                if name:lower():find("place") or name:lower():find("spawn") then
                    remote = r
                    return
                end
            end
        ...
        for i = 1, #descendants do
            ...
            if (obj:IsA("RemoteEvent") or obj:IsA("RemoteFunction")) then
                ...
                if name:find("place") or name:find("spawn") or name:find("tower") then
                    remote = obj
                    return
                end
            end
...
local function getUpgradeRemote()
    ...
    pcall(function()
        ...
        if getgenv().MacroRemoteCache and getgenv().MacroRemoteCache["upgrade"] then
        ...
        for i = 1, #descendants do
            local obj = descendants[i]
            if (obj:IsA("RemoteEvent") or obj:IsA("RemoteFunction")) then
                if obj.Name:lower():find("upgrade") then
                    remote = obj
                    return
                end
            end
...
-- Place a unit at position
local function placeUnit(unitName, position, direction)
    local success = false
    pcall(function()
        local placementRemote = getPlacementRemote()
        if not placementRemote then
            warn("[Auto Play] Placement remote not found")
            return
        end
        
        -- Create CFrame facing path direction
        local lookAt = position + direction * 10
        local cframe = CFrame.new(position, lookAt)
        
        -- Place the unit
        if placementRemote:IsA("RemoteFunction") then
            placementRemote:InvokeServer(unitName, cframe)
        else
            placementRemote:FireServer(unitName, cframe)
        end
        
        success = true
        print("[Auto Play] Placed " .. unitName .. " at " .. tostring(position))
    end)
    return success
end

-- Upgrade a tower
local function upgradeTower(tower, targetLevel)
    pcall(function()
        ...
        for i = currentLevel + 1, math.min(targetLevel, maxLevel) do
            task.wait(0.3)
...
local function hasGameStarted()
    ...
    pcall(function()
        ...
        local bottom = playerGui:FindFirstChild("Bottom")
        if not bottom then 
            started = true
            return 
        end
        
        local frame = bottom:FindFirstChild("Frame")
        if not frame then 
            started = true
            return 
        end
        ...
        if children[2] then
            ...
            if sub[6] then
                local btn = sub[6]:FindFirstChild("TextButton")
                if btn and btn:FindFirstChild("TextLabel") then
                    started = btn.TextLabel.Text ~= "Start"
                else
                    started = true
                end
            else
...
-- Main auto-play placement logic
local function autoPlaceUnits()
    if not getgenv().AutoPlayEnabled then return end
    ...
    pcall(function()
        ...
        task.wait(2) -- Small delay after game starts
        
        -- Get waypoints
        local waypoints = scanMapWaypoints()
        if #waypoints < 2 then
            warn("[Auto Play] No waypoints found")
            return
        end
        
        getgenv().AutoPlayPathWaypoints = waypoints
        local pathLength = getPathLength(waypoints)
        print("[Auto Play] Path length: " .. math.floor(pathLength) .. " studs")
        
        -- Get placement position
        local position, direction = getPositionAtPercentage(waypoints, getgenv().AutoPlayPercentage)
        if not position then
            warn("[Auto Play] Could not calculate placement position")
            return
        end
        
        print("[Auto Play] Placement position: " .. tostring(position))
        
        -- Get player slots
        local slots = getPlayerSlots()
        if #slots == 0 then
            warn("[Auto Play] No units in slots")
            return
        end
        
        -- Sort slots by cost (cheapest first for early game)
        table.sort(slots, function(a, b) return a.cost < b.cost end)
        
        -- Try to place each slot
        for _, slot in ipairs(slots) do
            if not getgenv().AutoPlayEnabled then break end
            
            -- Check if already placed
            local alreadyPlaced = false
            for _, placedUnit in ipairs(getgenv().AutoPlayPlacedUnits) do
                if placedUnit.name == slot.name and placedUnit.slot == slot.slot then
                    alreadyPlaced = true
                    break
                end
            end
            ...
            if not alreadyPlaced then
                ...
                local attempts = 0
                while getCurrentCash() < slot.cost and attempts < 300 do
                    if not getgenv().AutoPlayEnabled then return end
                    task.wait(1)
                    attempts = attempts + 1
                end
                
                if getCurrentCash() >= slot.cost then
                    -- Place the unit
                    local placed = placeUnit(slot.name, position, direction)
                    
                    if placed then
                        table.insert(getgenv().AutoPlayPlacedUnits, {
                            slot = slot.slot,
                            name = slot.name,
                            cost = slot.cost
                        })
                        
                        print("[Auto Play] Placed " .. slot.name .. " (Slot " .. slot.slot .. ") - Cost: $" .. slot.cost)
                        
                        -- Wait and upgrade if enabled
                        if getgenv().AutoPlayUpgradeAfterPlace and getgenv().AutoPlayUpgradeLevel > 0 then
                            task.wait(1)
                            
                            local tower = workspace.Towers:FindFirstChild(slot.name)
                            if tower and tower:FindFirstChild("Owner") and tower.Owner.Value == game:GetService("Players").LocalPlayer then
                                upgradeTower(tower, getgenv().AutoPlayUpgradeLevel)
                                print("[Auto Play] Upgraded " .. slot.name .. " to level " .. getgenv().AutoPlayUpgradeLevel)
                            end
                        end
                        
                        task.wait(0.5)
                    else
                        warn("[Auto Play] Failed to place " .. slot.name)
                    end
                else
                    print("[Auto Play] Timed out waiting for $" .. slot.cost .. " to place " .. slot.name)
                end
            end
        end
        
        print("[Auto Play] Finished placing all units")
    end)
end
...
task.spawn(function()
    ...
    while true do
        ...
        if getgenv().AutoPlayEnabled then
            ...
            -- Detect map change or new game
            if currentMap and currentMap ~= lastMap then
                print("[Auto Play] Map changed to: " .. currentMap)
                lastMap = currentMap
...
getgenv().TriggerAutoPlay = function()
    if not getgenv().AutoPlayEnabled then
        print("[Auto Play] Auto Play is disabled")
        return
    end
...
print("[Auto Play] System loaded successfully")
